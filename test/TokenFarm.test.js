...
contract('TokenFarm', ([owner, investor]) => {
    ...
    describe('Farming Tokens', async () => {
        it('rewards investors for staking mDai Tokens', async () => {
            ...
            assert.equal(result.toString(), tokens('100'), 'investor mockDai Wallet is correct, before staking');
            ...
            result = await daiToken.balanceOf(investor);
            assert.equal(result.toString(), tokens('0'), 'investor mockDai Wallet is correct, after staking');

            result = await daiToken.balanceOf(tokenFarm.address);
            assert.equal(result.toString(), tokens('100'), 'tokenFarm mockDai Wallet is correct, after staking');

            result = await tokenFarm.stakingBalance(investor);
            assert.equal(result.toString(), tokens('100'), 'stakingBalance is correct, after staking');

            result = await tokenFarm.isStaking(investor);
            assert.equal(result.toString(), 'true', 'isStaking is true, after staking');
        })
    })
